...
public class ElementAdapter extends BaseQuickAdapter<ZeroElementbeat, BaseViewHolder> {
    ...
    @Override
    protected void convert(BaseViewHolder helper, final ZeroElementbeat item) {
      ...
      /*  boolean sfans = item.isFollowStatus();
        ...
        if (!money.equals("") || !"".equals(money)) {
            helper.setText(R.id.element_gmoney, "¥" + item.getGmoney());
        } else {
            helper.setText(R.id.element_gmoney, "¥ 0.0");
        }
        ...
        helper.setOnClickListener(R.id.ll_element_item, new View.OnClickListener() {
            @Override
